...
- **图片保存位置**：对应日期归档目录下的 `photo/` 子目录，即 `YYYY/MM/DD/photo/`，缺失时创建。
...
3. 原图保存为工作目录下的 `YYYY/MM/DD/photo/<解析文件名去掉扩展名>_01.<原扩展名>`、`YYYY/MM/DD/photo/<解析文件名去掉扩展名>_02.<原扩展名>`，按输入顺序编号。保留原始图片内容和实际格式，不通过修改扩展名伪装格式。
4. 在解析文件中用 `photo/<图片文件名>` 相对路径引用对应原图，保证整个日期目录一起移动后仍可使用。例如解析 `2026/09/23/2026-09-23_10-00-00.md` 引用 `photo/2026-09-23_10-00-00_01.png`。原图用于追溯，正文仍须完整转录题目和必要图示信息。
